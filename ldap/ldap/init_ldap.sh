...
ldapadd -x -H ldap://localhost:1389 -D "cn=admin,dc=labcas,dc=jpl,dc=nasa,dc=gov" -w secret -f /tmp/ldap/add_user_ndeoyani.ldif
ldapadd -x -H ldap://localhost:1389 -D "cn=admin,dc=labcas,dc=jpl,dc=nasa,dc=gov" -w secret -f /tmp/ldap/add_user_test.ldif
